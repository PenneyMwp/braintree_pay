...
public class MainActivity extends BaseActivity {
    ...
    protected void init(Bundle onSaveInstanceState) {
        ...
        if (onSaveInstanceState != null) {
            if (onSaveInstanceState.containsKey(KEY_NONCE)) {
                mNonce = onSaveInstanceState.getParcelable(KEY_NONCE);
            }
        }
        mPaymentType = getIntent().getIntExtra(EXTRA_PAYMENT_TYPE, mPaymentType);
    }
    ...
    public void launchCards(View v) {
        ...
        intent.putExtra(EXTRA_COLLECT_DEVICE_DATA, Settings.shouldCollectDeviceData(this));
    ...
    public void createTransaction(View v) {
        ...
        intent.putExtra(CreateTransactionActivity.EXTRA_PAYMENT_METHOD_NONCE, mNonce);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            ...
            showDialog(((Exception) data.getSerializableExtra(DropInActivity.EXTRA_ERROR))
                    .getMessage(), new DialogInterface.OnClickListener() {
                @Override
    ...
    protected void onAuthorizationFetched() {
        enableButtons(true);
        if (mPaymentType == DROP_IN_REQUEST) {
            launchDropIn(null);
            finish();
        } else if (mPaymentType == GOOGLE_PAYMENT_REQUEST) {
            launchGooglePayment(null);
            finish();
        } else if (mPaymentType == CARDS_REQUEST) {
            launchCards(null);
            finish();
        } else if (mPaymentType == PAYPAL_REQUEST) {
            launchPayPal(null);
            finish();
        } else if (mPaymentType == VENMO_REQUEST) {
            launchVenmo(null);
            finish();
        } else if (mPaymentType == VISA_CHECKOUT_REQUEST) {
            launchVisaCheckout(null);
            finish();
        }
    }
